...
void ImgProcessor::createActions ()
{
    ...
    openFileAction->setStatusTip (tr("new a file"));
...
void ImgProcessor::ShowSaveFile ()
{
    if (curFileName.isEmpty ())    //原路径文件名
    {
		QString filename = QFileDialog::getSaveFileName(this, tr("保存文件"),
			QString(), tr("*.txt;;c++文件(*h *cpp *hpp);;*.doc"));//保存格式
        if (!filename.isEmpty ()) {
            QFile file(filename);
...
{
	QString filename = QFileDialog::getSaveFileName(this, tr("保存文件"),
		QString(), tr("文本文件(*.txt);;c++文件(*.h *.cpp *.hpp);;.doc"));
    if (!filename.isEmpty ()) {
        QFile file(filename);
        if (!file.open (QIODevice::WriteOnly)) {
            QMessageBox::critical (this, tr("错误"), tr("不能打开文件"));
...
void ImgProcessor::ShowFindDialog ()
{
    ...
    findNextBtn = new QPushButton(tr("find the next"));
...
void ImgProcessor::ShowSubDialog ()
{
    ...
    findNextBtn2 = new QPushButton(tr("find the next"));
